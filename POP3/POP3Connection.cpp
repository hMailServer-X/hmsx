...
namespace HM
{
   ...
   POP3Connection::GetCommand(ConnectionState currentState, String command)
   {
      ...
      if (command == PLATFORM_STRING("NOOP"))
         resolvedCommand = NOOP;
      if (command == PLATFORM_STRING("STLS"))
         resolvedCommand = STLS;
      else if (command == PLATFORM_STRING("USER"))
         resolvedCommand = USER;
      else if (command == PLATFORM_STRING("PASS"))
         resolvedCommand = PASS;
      else if (command == PLATFORM_STRING("HELP"))
         resolvedCommand = HELP;
      else if (command == PLATFORM_STRING("QUIT"))
         resolvedCommand = QUIT;
      else if (command == PLATFORM_STRING("STAT"))
         resolvedCommand = STAT;
      else if (command == PLATFORM_STRING("LIST"))
         resolvedCommand = LIST;
      else if (command == PLATFORM_STRING("RETR"))
         resolvedCommand = RETR;
      else if (command == PLATFORM_STRING("TOP"))
         resolvedCommand = TOP;
      else if (command == PLATFORM_STRING("RSET"))
         resolvedCommand = RSET;
      else if (command == PLATFORM_STRING("DELE"))
         resolvedCommand = DELE;
      else if (command == PLATFORM_STRING("UIDL"))
         resolvedCommand = UIDL;
      else if (command == PLATFORM_STRING("CAPA"))
   ...
   POP3Connection::ProtocolPASS_(const String &Parameter)
   {
      ...
      if (!account_)
      {
         if (username_.Find(PLATFORM_STRING("@")) == -1)
   ...
   POP3Connection::ProtocolLIST_(const String &sParameter)
   {
      ...
      if (sParameter.IsEmpty())
      {
         ...
         for(std::shared_ptr<Message> pMessage : messages_)
         {
            ...
            {
               sRow.Format(PLATFORM_STRING("%d %d\r\n"), index, pMessage->GetSize());
      ...
      else
      {
         ...
         if (pMessage)
            sResponse.Format(PLATFORM_STRING("+OK %d %d"), messageIndex, pMessage->GetSize());
         else
   ...
   POP3Connection::ProtocolUIDL_(const String &Parameter)
   {
      ...
      if (Parameter.IsEmpty())
      {
         ...
         sResponse.Format(PLATFORM_STRING("+OK %d messages (%I64d octets)\r\n"), iMessageCount, iTotalBytes);
         ...
         for(std::shared_ptr<Message> pMessage : messages_)
         {
            ...
            if (!pMessage->GetFlagDeleted())
            {
               sRow.Format(PLATFORM_STRING("%d %u\r\n"), index, pMessage->GetUID());
               sResponse.append(sRow);
            }
         }
      ...
      {  
         ...
         if (pMessage)
            sResponse.Format(PLATFORM_STRING("+OK %d %u"), messageIndex, pMessage->GetUID());
         else
            sResponse = "-ERR No such message";
      }
   ...
   {  
      ...
      catch (...)
      {
         ...
         sErrorMessage.Format(PLATFORM_STRING("Could not send file %s via socket since it does not exist."), fileName.c_str());
         ErrorManager::Instance()->ReportError(ErrorManager::Medium, 5063, "POP3Connection::_SendFile", sErrorMessage);
         return;
      }

      String responseTemp;
      responseTemp.Format(PLATFORM_STRING("+OK %d octets\r\n"), message->GetSize());
   ...
   POP3Connection::ProtocolTOP_(const String &Parameter)
   {
      ...
      long iNoOfLines = 0;
      int iSpacePos = Parameter.Find(PLATFORM_STRING(" "));
      if (iSpacePos >= 0)
      {
         Msg = Parameter.Mid(0, Parameter.Find(PLATFORM_STRING(" ")));
      ...
      else
      {
         ...
         String sResponse;
         sResponse.Format(PLATFORM_STRING("+OK %d octets"), pMessage->GetSize());
         EnqueueWrite_(sResponse);
   ...
   POP3Connection::ProtocolSTAT_(const String &sParameter)
   {
      ...
      String sResponse; 
      sResponse.Format(PLATFORM_STRING("+OK %d %I64d"), iMessageCount, iTotalSize);
   ...
   void
   POP3Connection::GetMailboxContents_(int &iNoOfMessages, long long &iTotalBytes)
   {
      iNoOfMessages = 0;
      iTotalBytes = 0;
