...
namespace HM
{
   ...
   IMAPCommandSEARCH::ExecuteCommand(std::shared_ptr<IMAPConnection> pConnection, std::shared_ptr<IMAPCommandArgument> pArgument)
   {
...

      {
         ...
         if (is_uid_)
            iCommandStartPos = sCommand.Find(PLATFORM_STRING(" "), 4) + 1;
         else
            iCommandStartPos = sCommand.Find(PLATFORM_STRING(" ")) + 1;
      ...
      if (messages.size() > 0)
      {
         ...
         for(MessagePair messagePair : vecMatchingMessages)
         {
            ...
            String sID;
            if (is_uid_)
               sID.Format(PLATFORM_STRING("%u"), pMessage->GetUID());
            else
               sID.Format(PLATFORM_STRING("%d"), index);
   ...
   //---------------------------------------------------------------------------()
{
   ...
   bool 
   IMAPCommandSEARCH::IsMessageRecent_(std::shared_ptr<IMAPConnection> pConnection, long long message_uid)
   {
