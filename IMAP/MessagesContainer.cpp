...
namespace HM
{
   ...
   MessagesContainer::GetMessages(long long account_id, long long folder_id)
   {
   ...
   std::shared_ptr<Messages>
   MessagesContainer::GetMessages(long long account_id, long long folder_id, std::set<long long> &recent_messages, bool update_recent_messages)
   {
   ...
   void 
   MessagesContainer::UncacheAccount(long long account_id)
   {
      messages_cache_.RemoveObject(Formatter::Format("{0}", account_id));
   }

   void
   MessagesContainer::SetFolderNeedsRefresh(long long folder_id)
   {
      auto cached_messages = messages_cache_.GetObject(folder_id);
      if (cached_messages == nullptr)
      {
