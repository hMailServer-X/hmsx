...
namespace HM
{
   ...
   String 
   FolderListCreator::GetIMAPFolderList(long long iAccountID, std::shared_ptr<IMAPFolders> pStartFolders, const String &sWildcard, const String &sPrefix) 
   {
   ...
   String 
   FolderListCreator::GetIMAPLSUBFolderList(long long iAccountID, std::shared_ptr<IMAPFolders> pStartFolders, const String &sWildcard, const String &sPrefix) 
   {
      std::vector<String> vecCurrentFolder;
      std::vector<String> vecMatchingFolders;
   ...
   void
   FolderListCreator::CreateIMAPFolderList_(long long iAccountID, std::shared_ptr<IMAPFolders> pStartFolders, const String &sWildcard, bool bOnlySubscribed, const String &sPrefix, std::vector<String> &vecCurrentFolder, std::vector<String> &vecMatchingFolders) 
   {
   ...
   FolderListCreator::CreateFolderLine_(std::shared_ptr<IMAPFolder> currentFolder, bool bOnlySubscribed, bool hasSubFolders, String &sFullPath, const String &sWildcard, bool isSelectable, String hierarchyDelimiter)
   {
      ...
      if (bOnlySubscribed && (!currentFolder || currentFolder->GetIsSubscribed()))
         sFolderLine.Format(PLATFORM_STRING("* LSUB (%s) \"%s\" %s"), nameAttributes.c_str(), hierarchyDelimiter.c_str(), sFullPath.c_str());
      else if (!bOnlySubscribed) 
         sFolderLine.Format(PLATFORM_STRING("* LIST (%s) \"%s\" %s"), nameAttributes.c_str(), hierarchyDelimiter.c_str(), sFullPath.c_str());
