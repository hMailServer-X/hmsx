...
namespace HM
{
   ...
   public:
      IMAPSortCachedDateTime(std::map<long long, DateTime> &map) :
         date_time_info_(map)
      {
      }
   ...
   private:
      std::map<long long, DateTime> &date_time_info_;
   };   

   class IMAPSortHeaderField 
   {
   public:
      IMAPSortHeaderField(std::map<long long, String> &mapHeaderFields, String character_set) :
      ...
      bool operator()(const std::pair<int, std::shared_ptr<Message> > p1, const std::pair<int, std::shared_ptr<Message> >  p2)
      {
         ...
         if (character_set_.CompareNoCase(PLATFORM_STRING("US-ASCII")) == 0)
         {
   ...
   private:
      ...
      std::map<long long, String> &_mapHeaderFields;
   ...
   IMAPSort::Sort(std::shared_ptr<IMAPConnection> pConnection, std::vector<std::pair<int, std::shared_ptr<Message> > > &vecMessages, String character_set, std::shared_ptr<IMAPSortParser> pParser)
   {
      ...
      if (iterSortType != vecSortTypes.end())
      {
         ...
         std::map<long long, String > databaseMetaData;
         ...
         std::map<long long, String> mapHeaderFields;
         ...
         if (sortField == Arrival) 
         {
            ...
            std::map<long long, DateTime> mapDateTimeInfo;
   ...
   IMAPSort::CacheHeaderFields_(std::shared_ptr<IMAPConnection> pConnection,
                                ...
                                const std::map<long long, String > &databaseMetaData, 
                                SortField &sortField,
                                std::map<long long, String> &mapHeaderFields)
   {
      ...
      for (; iter != iterEnd; iter++)
      {
         ...
         std::map<long long, String >::const_iterator dbMetaIter = databaseMetaData.find(p1->GetID());
         ...
         {
            sFieldValue.TrimLeft(PLATFORM_STRING("\""));
            sFieldValue.TrimLeft(PLATFORM_STRING("<"));
         }
