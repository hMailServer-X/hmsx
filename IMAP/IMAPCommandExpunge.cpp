...
namespace HM
{
   ...
   IMAPCommandEXPUNGE::ExecuteCommand(std::shared_ptr<IMAPConnection> pConnection, std::shared_ptr<IMAPCommandArgument> pArgument)
   {
      ...
      std::vector<long long> expunged_messages_uid;
      std::vector<long long> expunged_messages_index;
      ...
      {
         String sTemp;
         sTemp.Format(PLATFORM_STRING("* %d EXPUNGE\r\n"), (*iterExpunged));
         sResponse += sTemp;
         iterExpunged++;
      }
      ...
      if (!expunged_messages_uid.empty())
      {
         ...
         for (long long messageUid : expunged_messages_uid)
         {
