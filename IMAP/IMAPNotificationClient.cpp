...
namespace HM
{
   ...
   void 
   IMAPNotificationClient::SubscribeMessageChanges(long long accountID, long long folderID)
   {
      assert(accountID >= 0);
      assert(folderID > 0);
   ...
   IMAPNotificationClient::SendCachedNotifications(bool send_expunge)
   {
      ...
      std::set<long long> flagMessages;
         ...
         case ChangeNotification::NotificationMessageFlagsChanged:
            {
               ...
               for(long long messageID : changeNotification->GetAffectedMessages())
               {
   ...
   IMAPNotificationClient::SendChangeNotification_(std::shared_ptr<ChangeNotification> pChangeNotification)
   {
      ...
      case ChangeNotification::NotificationMessageFlagsChanged:
         {
            // Send flag notification
            std::set<long long> affectedMessages;
               for(long long messageID : pChangeNotification->GetAffectedMessages())
            {
   ...
   IMAPNotificationClient::SendEXPUNGE_(const std::vector<long long> & vecMessages)
   {
      std::shared_ptr<IMAPConnection> connection = parent_connection_.lock();
      if (!connection)
         return;
      ...
      String sResponse;
      for(long long messageIndex : vecMessages)
         sResponse.AppendFormat(PLATFORM_STRING("* %I64d EXPUNGE\r\n"), messageIndex);
   ...
   void 
   IMAPNotificationClient::SendFLAGS_(const std::set<long long> & vecMessages)
   {
      std::shared_ptr<IMAPConnection> connection = parent_connection_.lock();
      if (!connection)
         return;

      for(long long messageID : vecMessages)
      {
         String sResponse;
