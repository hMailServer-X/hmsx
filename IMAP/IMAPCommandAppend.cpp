...
namespace HM
{
   ...
   IMAPCommandAppend::ExecuteCommand(std::shared_ptr<IMAPConnection> pConnection, std::shared_ptr<IMAPCommandArgument> pArgument)
   {
      ...
      {
         String sMessage;
         sMessage.Format(PLATFORM_STRING("Message size exceeds fixed maximum message size. Size: %d KB, Max size: %d KB"), 
            bytes_left_to_receive_ / 1024, maxMessageSizeKB);
      ...
      long long lFolderID = destination_folder_->GetID();
   ...
   IMAPCommandAppend::Finish_(std::shared_ptr<IMAPConnection> pConnection)
   {
      ...
      // Set message flags.
      bool bSeen = (flags_to_set_.FindNoCase(PLATFORM_STRING("\\Seen")) >= 0);
      bool bDeleted = (flags_to_set_.FindNoCase(PLATFORM_STRING("\\Deleted")) >= 0);
      bool bDraft = (flags_to_set_.FindNoCase(PLATFORM_STRING("\\Draft")) >= 0);
      bool bAnswered = (flags_to_set_.FindNoCase(PLATFORM_STRING("\\Answered")) >= 0);
      bool bFlagged = (flags_to_set_.FindNoCase(PLATFORM_STRING("\\Flagged")) >= 0);
