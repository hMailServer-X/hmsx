...
namespace HM
{
   ...
   IMAPCommandUID::ExecuteCommand(std::shared_ptr<IMAPConnection> pConnection, std::shared_ptr<IMAPCommandArgument> pArgument)
   {
      ...
      if (sTypeOfUID.CompareNoCase(PLATFORM_STRING("FETCH")) == 0)
      {
      ...
      else if (sTypeOfUID.CompareNoCase(PLATFORM_STRING("COPY")) == 0)
      {
      ...
      else if (sTypeOfUID.CompareNoCase(PLATFORM_STRING("STORE")) == 0)
      {
         if (pParser->WordCount() < 4)
            return IMAPResult(IMAPResult::ResultBad, "Command requires at least 3 parameters.");
      ...
      else if (sTypeOfUID.CompareNoCase(PLATFORM_STRING("SEARCH")) == 0)
      {
      ...
      else if (sTypeOfUID.CompareNoCase(PLATFORM_STRING("SORT")) == 0)
      {
      ...
      long lSecWordStartPos = sCommand.Find(PLATFORM_STRING(" "), 5) + 1;
      long lSecWordEndPos = sCommand.Find(PLATFORM_STRING(" "), lSecWordStartPos);
