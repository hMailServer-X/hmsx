...
namespace HM
{
   ...
   IMAPFetch::DoAction(std::shared_ptr<IMAPConnection> pConnection, int messageIndex, std::shared_ptr<Message> pMessage, const std::shared_ptr<IMAPCommandArgument> pArgument)
   {
      ...
      sOutput.Format(PLATFORM_STRING("* %d FETCH ("), messageIndex);

      // We should always show UID when client is issuing UID fetch..
      if (parser_->GetShowUID() || GetIsUID()) 
      {
         String sUIDTag;
         sUIDTag.Format(PLATFORM_STRING("UID %u"), pMessage->GetUID());
      ...
      {
         String sTemp;
         sTemp.Format(PLATFORM_STRING("RFC822.SIZE %d"), pMessage->GetSize());
      ...
      if (vecPartsToPeekAt.size() > 0)
      {
         ...
         while (iter != vecPartsToPeekAt.end())   
         {
            ...
            if (iOctetStart == -1 && iOctetCount == -1)
               sPartIdentifier.Format(PLATFORM_STRING("%s"), oPart.GetDescription().c_str());
            else
               sPartIdentifier.Format(PLATFORM_STRING("%s<%d>"), oPart.GetDescription().c_str(), iOctetStart, iOctetCount);

            if (pBuffer->GetSize() > 0)
            {
               // Send part size information
               String sTemp;
               sTemp.Format(PLATFORM_STRING("%s {%d}\r\n"), sPartIdentifier.c_str(), pBuffer->GetSize()); // Add 2 for trailing newline.
            ...
            else
            {
               ...
               String sTemp;
               sTemp.Format(PLATFORM_STRING("%s \"\""), sPartIdentifier.c_str());
               AppendOutput_(sOutput, sTemp);
            }
   ...
   IMAPFetch::CreateEnvelopeStructure_(MimeHeader& oHeader)
   {
      ...
      {  // BEGIN DATE
         ...
         if (pField)
         {
            ...
            int iStartPara = sDate.Find(PLATFORM_STRING("("));
            if (iStartPara >= 0)
   ...
   IMAPFetch::IteratePartRecursive_(std::shared_ptr<MimeBody> oPart, bool includeExtensionData, int iRecursion)
   {
      ...
      if (oPart->IsMultiPart())
      {
         ...
         // Add subtype stamp.
			...
			String sTemp;
         sTemp.Format(PLATFORM_STRING(" \"%s\""), sType.c_str());
			sResult += sTemp;

			if (includeExtensionData)
			{
				String sBoundary = oPart->GetBoundary();
            sTemp.Format(PLATFORM_STRING(" (\"BOUNDARY\" \"%s\") NIL NIL"), sBoundary.c_str());
				sResult += sTemp;
			}
   ...
   IMAPFetch::GetPartStructure_(std::shared_ptr<MimeBody> oPart, bool includeExtensionData, int iRecursion)   
   {
      ...
      if (oPart->IsAttachment())
         sBodyParams.Format(PLATFORM_STRING("(\"NAME\" \"%s\")"), oPart->GetRawFilename().c_str());
      else
      {
         ...
         sBodyParams.Format(PLATFORM_STRING("(\"CHARSET\" \"%s\")"), sCharset.c_str());
      }
      ...
      String sSize;
      sSize.Format(PLATFORM_STRING("%d"), oPart->GetContentLength());
      ...
      if (oPart->IsEncapsulatedRFC822Message())
      {
         ...
         */
         ...
         try
         {
            ...
            String sLines;
            sLines.Format(PLATFORM_STRING("%d"), lLines);
      ...
      if (sMainType.CompareNoCase(PLATFORM_STRING("TEXT")) == 0)
      {
         /*
         ...
         */
         ...
         String sTemp;
         sTemp.Format(PLATFORM_STRING(" %d"), lLines);
      ...
      if (oPart->IsAttachment())
      {
         ...
         int iSemiPos = sDisposition.Find(PLATFORM_STRING(";"));
         if (iSemiPos > 0)
         ...
         String sTemp;
         sTemp.Format(PLATFORM_STRING("NIL (\"%s\" (\"FILENAME\" \"%s\"))"), sDisposition.c_str(), oPart->GetRawFilename().c_str());
         sResult += " " + sTemp;
      }
   ...
   IMAPFetch::CreateEmailStructure_(const String &sField)
   {
      ...
      while (iterElement != vecAddresses.end())
      {
         ...
         else
         {
            ...
            sThisAddress.Format(PLATFORM_STRING("(\"%s\" NIL \"%s\" \"%s\")"), sOutName.c_str(), sOutMailboxName.c_str(), sOutDomainName.c_str());
         }
