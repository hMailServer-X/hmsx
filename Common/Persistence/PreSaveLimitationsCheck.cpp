...
namespace HM   
{
   ...
   PreSaveLimitationsCheck::GetDomain(long long id)
   {
   ...
   PreSaveLimitationsCheck::CheckLimitations(PersistenceMode mode, std::shared_ptr<Account> account, String &resultDescription)
   {
      ...
      if (account->GetAddress().FindOneOf(PLATFORM_STRING(" \"")) != -1)
      {
      ...
      if (domain->GetMaxSizeMB() > 0)
      {
         ...
         long long currentSize = PersistentDomain::GetAllocatedSize(domain);
      ...
      if (address.Find(PLATFORM_STRING("\\")) >= 0 || address.Find(PLATFORM_STRING("/")) >= 0)
      {
   ...
   bool 
   PreSaveLimitationsCheck::GetDuplicateExist(std::shared_ptr<Domain> domain, ObjectType objectType, long long objectID, const String &objectName)
   {
   ...
   PreSaveLimitationsCheck::CheckLimitations(PersistenceMode mode, std::shared_ptr<Domain> domain, String &resultDescription)
   {
      ...
      if (domain->GetUsePlusAddressing() && domain->GetPlusAddressingChar() == PLATFORM_STRING(""))
      {
