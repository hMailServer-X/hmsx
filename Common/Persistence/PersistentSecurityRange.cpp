...
namespace HM
{
   ...
   PersistentSecurityRange::SaveObject(std::shared_ptr<SecurityRange> pSR, String &result,  PersistenceMode mode)
   {
      ...
      else
      {
         ...
         String sWhere;
         sWhere.Format(PLATFORM_STRING("rangeid = %I64d"), pSR->GetID());
   ...
   bool
   PersistentSecurityRange::ReadObject(std::shared_ptr<SecurityRange> pSR, long long lDBID)
   {
      SQLCommand command(PLATFORM_STRING("select * from hm_securityranges where rangeid = @RANGEID"));
   ...
   {
      SQLCommand command(PLATFORM_STRING("select * from hm_securityranges where rangename = @RANGENAME"));
   ...
   PersistentSecurityRange::ReadMatchingIP(const IPAddress &ipaddress)
   {
      ...
      if (ipaddress.GetType() == IPAddress::IPV4)
      {
         ...
         sSQL.Format(PLATFORM_STRING("select * from hm_securityranges where %s >= rangelowerip1 and %s <= rangeupperip1 and rangelowerip2 IS NULL and rangeupperip2 IS NULL order by rangepriorityid desc"), 
      ...
      else
      {
         ...
         SQLCommand command(PLATFORM_STRING("select * from hm_securityranges where rangelowerip2 is not null order by rangepriorityid desc"));
   ...
   {
      SQLCommand command(PLATFORM_STRING("delete from hm_securityranges where rangeexpires = 1 AND rangeexpirestime < @TIME"));
