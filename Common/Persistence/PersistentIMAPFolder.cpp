...
namespace HM
{
   ...
   bool
   PersistentIMAPFolder::DeleteByAccount(long long iAccountID)
   {
      if (iAccountID <= 0)
         return false;
   ...
   */
   ...
   PersistentIMAPFolder::DeleteObject(std::shared_ptr<IMAPFolder> pFolder, bool forceDelete)
   {
      ...
      bool isInbox = pFolder->GetParentFolderID() == -1 && pFolder->GetFolderName().CompareNoCase(PLATFORM_STRING("Inbox")) == 0;
   ...
   PersistentIMAPFolder::SaveObject(std::shared_ptr<IMAPFolder> pFolder)
   {
      ...
      else
      {
         ...
         String sWhere;
         sWhere.Format(PLATFORM_STRING("folderid = %I64d"), pFolder->GetID());
   ...
   long long 
   PersistentIMAPFolder::GetUserInboxFolder(long long accountID)
   {
      ...
      {
         String message;
         message.Format(PLATFORM_STRING("The inbox for account %I64d could not be looked up"), accountID);
         ErrorManager::Instance()->ReportError(ErrorManager::Medium, 5206, "PersistentIMAPFolder::GetUserInboxFolder", message);
         return 0;
      }

      long long folderID = pRS->GetInt64Value("folderid");
   ...
   PersistentIMAPFolder::GetExistsFolderContainingCharacter(String theChar)
   {
      ...
      SQLCommand command(PLATFORM_STRING("select count(*) as c from hm_imapfolders where foldername like '%" + theChar + "%'"));
   ...
   unsigned int 
   PersistentIMAPFolder::GetCurrentUID_(long long folderID)
   {
      if (folderID == 0)
         return 0;
      ...
      {
         String message;
         message.Format(PLATFORM_STRING("Current UID for folder %I64d could not be looked up"), folderID);
      ...
      {
         String message;
         message.Format(PLATFORM_STRING("Current UID for folder %I64d could not be looked up. Folder does not eixst."), folderID);
         ErrorManager::Instance()->ReportError(ErrorManager::Medium, 5207, "PersistentIMAPFolder::GetCurrentUID_", message);
   ...
   bool 
   PersistentIMAPFolder::IncreaseCurrentUID_(long long folderID)
   {
   ...
   unsigned int 
   PersistentIMAPFolder::GetUniqueMessageID(long long accountID, long long folderID)
   {
      if (folderID == 0)
         return 0;
