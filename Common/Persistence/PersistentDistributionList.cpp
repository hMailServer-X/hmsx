...
namespace HM
{
   ...
   bool
   PersistentDistributionList::ReadObject(std::shared_ptr<DistributionList> pDistList, long long iID)
   {
   ...
   PersistentDistributionList::SaveObject(std::shared_ptr<DistributionList> pDistList, String &sErrorMessage, PersistenceMode mode)
   {
      ...
      else
      {
         ...
         String sWhere;
         sWhere.Format(PLATFORM_STRING("distributionlistid = %I64d"), pDistList->GetID());
