...
namespace HM
{
   ...
   {
      long long iDomainID = pDomain->GetID();
      assert(iDomainID);
   ...
   bool
   PersistentDomain::ReadObject(std::shared_ptr<Domain> pDomain, long long ObjectID)
   {
   ...
   PersistentDomain::SaveObject(std::shared_ptr<Domain> pDomain, String &sErrorMessage, PersistenceMode mode)
   {
      ...
      else
      {
         ...
         String sWhere;
         sWhere.Format(PLATFORM_STRING("domainid = %I64d"), pDomain->GetID());
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      long long iSize = pRS->GetInt64Value("size");
