...
namespace HM
{
   ...
   PersistentMessageMetaData::GetMessagesToIndex(bool quickIndex = false)
   {
      ...
      if (quickIndex == true) 
      {
         ...
         String whereClause;
         whereClause.Format(PLATFORM_STRING("hm_messages.messagetype = 2 AND hm_messages.messageid > %d AND hm_message_metadata.metadata_id IS NULL"), oldestMessageToIndex);
   ...
   void
   PersistentMessageMetaData::GetMetaData(int accountID, int folderID, const String &headerField, std::map<long long, String > &result)
   {
      ...
      String whereClause;
      whereClause.Format(PLATFORM_STRING("metadata_accountid = %d and metadata_folderid = %d"), accountID, folderID);
      ...
      {
         String value;
         long long messageID = pRS->GetInt64Value("metadata_messageid");     
