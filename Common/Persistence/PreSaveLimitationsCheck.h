...
namespace HM
{
   ...
   private:
      ...
      static std::shared_ptr<Domain> GetDomain(long long id);
      static bool DuplicateError(String &resultDescription);

      static bool GetDuplicateExist(std::shared_ptr<Domain> domain, ObjectType objectType, long long objectID, const String &objectName);
   };
}