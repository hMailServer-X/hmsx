...
namespace HM
{
   ...
   PersistentMessage::DeleteObject(std::shared_ptr<Message> pMessage)
   {
      ...
      long long iMessageID = pMessage->GetID();

      if (iMessageID > 0)
      {
         SQLCommand command(PLATFORM_STRING("delete from hm_messages where messageid = @MESSAGEID"));
   ...
   bool
   PersistentMessage::GetMessageID(const String &fileName, long long &messageID, bool &isPartialFilename)
   {
      messageID = 0;
   ...
   PersistentMessage::DeleteFile(std::shared_ptr<const Account> account, std::shared_ptr<Message> pMessage)
   {
      ...
      String sLogMessage;
      sLogMessage.Format(PLATFORM_STRING("Deleting message file."));
      ...
      if (pMessage->GetID() > 0)
      {
         ...
         sErrorMessage.Format(PLATFORM_STRING("Tried to delete the file %s even though the message was not deleted."), messageFile.c_str());
   ...
   PersistentMessage::ReadObject(std::shared_ptr<DALRecordset> pRS, std::shared_ptr<Message> pMessage, bool bReadRecipients)
   {
      ...
      if (pMessage->GetFolderID() > 0 && pMessage->GetUID() == 0)
      {
         ...
         String sErrorMessage;
         sErrorMessage.Format(PLATFORM_STRING("The message %I64d in mailbox %I64d in account %I64d has no UID set."), 
   ...
   bool 
   PersistentMessage::ReadObject(std::shared_ptr<Message> pMessage, long long ObjectID)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   ...
   bool
   PersistentMessage::LockObject(long long ObjectID)
   {
      ASSERT(ObjectID > 0);
   ...
   PersistentMessage::CopyFromQueueToInbox(std::shared_ptr<Message> sourceMessage, std::shared_ptr<const Account> destinationAccount)
   {
      ...
      // Locate the inbox ID
      long long  inboxID = CacheContainer::Instance()->GetInboxIDCache().GetUserInboxFolder(destinationAccount->GetID());
      if (inboxID == 0)
      {
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      */
      if (pMessage->GetFolderID() > 0 && pMessage->GetUID() == 0 )
      {
         ...
         {
            String errorMessage;
            errorMessage.Format(PLATFORM_STRING("Failed to generate UID for message. Account ID: %I64d, Folder ID: %I64d"), pMessage->GetAccountID(), pMessage->GetFolderID());
      ...
      else
      {
         ...
         String sWhere;
         sWhere.Format(PLATFORM_STRING("messageid = %I64d"), pMessage->GetID());
   ...
   bool
   PersistentMessage::SetNextTryTime(long long iMessageID, bool bUpdateNoOfTries, long lNoOfMinutes)
   {
      ...
      String sWhereClause = PLATFORM_STRING("where messageid = @MESSAGEID and (messagetype = 1 or messagetype = 3)");
   ...
   PersistentMessage::EnsureFileExistance(std::shared_ptr<const Account> account, std::shared_ptr<Message> pMessage)
   {
      ...
      // Replace macros
      sMessageBody.Replace(PLATFORM_STRING("%MACRO_FILE%"), sFileName);

      String sErrorMessage;
      sErrorMessage.Format(PLATFORM_STRING("From: Postmaster\r\n")
                           PLATFORM_STRING("Subject: %s\r\n")
                           PLATFORM_STRING("Date: %s\r\n")
                           PLATFORM_STRING("\r\n")
                           PLATFORM_STRING("%s")
                           PLATFORM_STRING("\r\n"),
   ...
   bool
   PersistentMessage::DeleteByAccountID(long long iAccountID)
   {
      ...
      while (!pRS->IsEOF())
      {
         ...
         {
            String sErrorMessage;
            sErrorMessage.Format(PLATFORM_STRING("Failed to delete file %s while deleting messages in account %I64d"), sFileName.c_str(), iAccountID);
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      while (bMoreData)
      {
         ...
         iHeaderEnd = sHeaderData.Find(PLATFORM_STRING("\r\n\r\n"));
