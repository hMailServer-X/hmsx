...
namespace HM
{
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      String sSQL;
      sSQL.Format(PLATFORM_STRING("select * from hm_greylisting_triplets where glipaddress1 %s and glipaddress2 %s and glsenderaddress = @SENDERADDRESS and glrecipientaddress = @RECIPIENTADDRESS"), 
   ...
   void 
   PersistentGreyList::IncreaseBlocked(long long iTripletID)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
