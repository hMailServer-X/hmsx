...
namespace HM
{
   ...
   void
   PersistentFetchAccount::Lock(long long ID)
   {
   ...
   void 
   PersistentFetchAccount::Unlock(long long ID)
   {
   ...
   PersistentFetchAccount::UnlockAll()
   {
	...
	bool
	PersistentFetchAccount::IsLocked(long long ID)
	{
   ...
   void
   PersistentFetchAccount::DeleteByAccountID(long long iAccountID)
   {
   ...
   PersistentFetchAccount::SaveObject(std::shared_ptr<FetchAccount> pFA)
   {
      ...
      {
         String sWhere;
         sWhere.Format(PLATFORM_STRING("faid = %I64d"), pFA->GetID());
   ...
   void 
   PersistentFetchAccount::SetRetryNow(long long iFAID)
   {
   ...
   PersistentFetchAccount::SetNextTryTime(std::shared_ptr<FetchAccount> pFA)
   {
      ...
      case DatabaseSettings::TypeMYSQLServer:
         command.SetQueryString(PLATFORM_STRING("update hm_fetchaccounts set fanexttry = DATE_ADD(CONCAT(CURDATE(), ' ', CURTIME()), INTERVAL faminutes MINUTE) where faid = @FAID"));
         break;
      case DatabaseSettings::TypeMSSQLServer:
      case DatabaseSettings::TypeMSSQLCompactEdition:
         command.SetQueryString(PLATFORM_STRING("update hm_fetchaccounts set fanexttry = DATEADD(mi, faminutes, GETDATE()) where faid = @FAID"));
         break;
      case DatabaseSettings::TypePGServer:
         command.SetQueryString(PLATFORM_STRING("update hm_fetchaccounts set fanexttry = current_timestamp + INTERVAL '@MINUTES minutes' where faid = @FAID"));
