...
static const TCHAR szXMLPIOpen[] = PLATFORM_STRING("<?");
static const TCHAR szXMLPIClose[] = PLATFORM_STRING("?>");
static const TCHAR szXMLCommentOpen[] = PLATFORM_STRING("<!--");
static const TCHAR szXMLCommentClose[] = PLATFORM_STRING("-->");
static const TCHAR szXMLCDATAOpen[] = PLATFORM_STRING("<![CDATA[");
static const TCHAR szXMLCDATAClose[] = PLATFORM_STRING("]]>");
...
LPTSTR _tagXMLNode::LoadAttributes( LPCTSTR pszAttrs , LPPARSEINFO pi /*= &piDefault*/)
{
	...
	while( xml && *xml )
	{
		if( xml = _tcsskip( xml ) )
		{
			...
			// XML Attr Name
			TCHAR* pEnd = _tcspbrk( xml, PLATFORM_STRING(" =") );
			if( pEnd == NULL ) 
			{
				// error
               ...
               pi->error_string.Format(PLATFORM_STRING("<%s> attribute has error "), name.c_str());
				}
				return NULL;
			}
				...
				if( *xml == '=' )
				{
					if( xml = _tcsskip( ++xml ) )
					{
						...
						else
						{
							...
							// none quote mode
							//pEnd = _tcsechr( xml, ' ', '\\' );
							pEnd = _tcsepbrk( xml, PLATFORM_STRING(" >"), pi ? pi->escape_value : 0 );
						}
...
LPTSTR _tagXMLNode::LoadProcessingInstrunction( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
	...
	if( doc )
	{
		...
		TCHAR* pTagEnd = _tcspbrk( xml, PLATFORM_STRING(" ?>") );
...
LPTSTR _tagXMLNode::LoadComment( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
	...
	if( par )
	{
		...
		node->name = PLATFORM_STRING("#COMMENT");
...
LPTSTR _tagXMLNode::LoadCDATA( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
	...
	if( par )
	{
		...
		node->name = PLATFORM_STRING("#CDATA");
		_SetString( xml, end, &node->value, FALSE );
...
LPTSTR _tagXMLNode::Load( LPCTSTR pszXml, LPPARSEINFO pi /*= &piDefault*/ )
{
	...
	if( xml = LoadAttributes( xml, pi ) )
	{
		...
		if( *xml == chXMLTagPre )
		{
			...
			else
			{
				...
				if( pi->erorr_occur == false ) 
				{
					...
					pi->error_string = PLATFORM_STRING("Element must be closed.");
				}
				// not wel-formed tag
				return NULL;
			}
		...
		//                        ^- current pointer
		{
			...
			//if( this->value.IsEmpty() || this->value == PLATFORM_STRING("") )
			if( XIsEmptyString( value ) )
			{
				...
				if( pEnd == NULL ) 
				{
                  ...
                  pi->error_string.Format(PLATFORM_STRING("%s must be closed with </%s>"), name.c_str(), name.c_str());
					}
					...
					if( xml = _tcsskip( xml ) )
					{
						...
						if( pEnd == NULL ) 
						{
							if( pi->erorr_occur == false ) 
							{
								...
								pi->error_pointer = xml;
								pi->error_code = PIE_NOT_CLOSED;
								pi->error_string.Format(PLATFORM_STRING("it must be closed with </%s>"), name.c_str() );
							}
							// error
							return NULL;
						}
						...
						else
						{
							...
							if( pi->force_parse == false )
							{
								...
								if( pi->erorr_occur == false ) 
								{
									...
									pi->error_string.Format(PLATFORM_STRING("'<%s> ... </%s>' is not wel-formed."), name.c_str(), closename.c_str() );
								}
								return NULL;
							}
						}
...
LPTSTR _tagXMLDocument::Load( LPCTSTR pszXml, LPPARSEINFO pi /*= NULL*/ )
{
	...
	if( (end = node->Load( pszXml, pi )) == NULL )
	{
      ...
      if (pi->erorr_occur == true)
      {
         ...
         sErrorMessage.Format(PLATFORM_STRING("The XML data stream contains formatting errors. Code: %d, Error: %s"), pi->error_code, pi->error_string.c_str());
...
{
	LPXNode node = GetChild( name );
	return (node != NULL)? node->GetText(opt) : PLATFORM_STRING("");
}
