...
namespace HM
{
   ...
   ServerStatus::GetUnsortedMessageStatus() const
   {
      ...
      while (!pRS->IsEOF())
      {
         ...
         sLine.Format(PLATFORM_STRING("%I64d\t%s\t%s\t%s\t%s\t%s\t%d\t%d"), lMessageID, sCreateTime.c_str(), sFrom.c_str(), sTo.c_str(), sNextTryTime.c_str(), sFileName.c_str(), bLocked, lNoOfTries);
