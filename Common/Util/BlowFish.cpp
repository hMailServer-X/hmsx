...
namespace HM
{
   ...
   BlowFishEncryptorTester::Test()
   {
      ...
      if (pBL->DecryptFromString(pBL->EncryptToString(PLATFORM_STRING("Hejsan"))).CompareNoCase(PLATFORM_STRING("Hejsan")) != 0)
         assert(0);

      if (pBL->DecryptFromString(pBL->EncryptToString(PLATFORM_STRING("dcidjea"))).CompareNoCase(PLATFORM_STRING("dcidjea")) != 0)
         assert(0);
