...
namespace HM
{
   ...
   bool DateTime::ConvertStringToDate(const std::wstring& str, DATE& result)
   {
       struct tm tmDate = {};
       if (swscanf_s(str.c_str(), L"%d-%d-%d %d:%d:%d",
           &tmDate.tm_year, &tmDate.tm_mon, &tmDate.tm_mday,
           &tmDate.tm_hour, &tmDate.tm_min, &tmDate.tm_sec) == 6)
       {
           tmDate.tm_year -= 1900; // Year is offset by 1900 in tm structure
           tmDate.tm_mon -= 1;     // Month is 0-based in tm structure
           result = _mkgmtime(&tmDate); // Convert to DATE
           return true;
       }
       return false;
   }
   ...
   // Modified - D - To remove ATL/COM specifics
   BOOL DateTime::ParseDateTime(LPCTSTR lpszDate, DWORD dwFlags, LCID lcid)
   {

       // Konvertiere LPCTSTR in eine std::wstring
       std::wstring wideDate(lpszDate);

       // Benutzerdefinierte Funktion zur Zeichenfolgenkonvertierung
       if (ConvertStringToDate(wideDate, dt_))
       {
           SetStatus(valid);
           return TRUE;
       }
