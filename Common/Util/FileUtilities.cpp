...
namespace HM
{
   const String FileUtilities::PathSeparator = PLATFORM_STRING("\\");
      ...
      for (int i = 1; i <= iMaxNumberOfTries; i++)
      {
         ...
         if (error_code)
         {
            ...
            if (i == iMaxNumberOfTries)
            {
               ...
               sErrorMessage.Format(PLATFORM_STRING("Could not delete the file %s. Tried 5 times without success."), FileName.c_str());
   ...
   FileUtilities::Copy(const String &sFrom, const String &sTo, bool bCreateMissingDirectories)
   {
      ...
      {
         String sToPath = sTo.Mid(0, sTo.ReverseFind(PLATFORM_STRING("\\")));
      ...
      for (int i = 1; i <= iMaxNumberOfTries; i++)
      {
         ...
         if (i == iMaxNumberOfTries)
         {
            ...
            String sErrorMessage;
            sErrorMessage.Format(PLATFORM_STRING("Could not copy the file %s to %s. Tried 5 times without success."), sFrom.c_str(), sTo.c_str());
   ...
   FileUtilities::Move(const String &sFrom, const String &sTo, bool overwrite)
   {
      ...
      for (int i = 1; i <= iMaxNumberOfTries; i++)
      {
         ...
         if (i == iMaxNumberOfTries)
         {
            ...
            String sErrorMessage;
            sErrorMessage.Format(PLATFORM_STRING("Could not move the file %s to %s. Tried 5 times without success."), sFrom.c_str(), sTo.c_str());
   ...
   //---------------------------------------------------------------------------()
   {
      int iLastSlash = FileName.ReverseFind(PLATFORM_STRING("\\"));
   ...
   //---------------------------------------------------------------------------()
   {
      int iLastSlash = sFullPath.ReverseFind(PLATFORM_STRING("\\"));
   ...
   {
      String sTmpFile;
      sTmpFile.Format(PLATFORM_STRING("%s\\%s.tmp"), IniFileSettings::Instance()->GetTempDirectory().c_str(), GUIDCreator::GetGUID().c_str());
      return sTmpFile;
   }
   ...
   FileUtilities::CreateDirectory(const String &sName)
   {
      ...
      for (int i = 1; i <= iMaxNumberOfTries; i++)
      {
         ...
         if (i == iMaxNumberOfTries)
         {
            ...
            String sErrorMessage;
            sErrorMessage.Format(PLATFORM_STRING("Could not create the directory %s. Tried 5 times without success."), sName.c_str());
   ...
   FileUtilities::DeleteDirectoriesInDirectory(const String &sDirName)
   {
      for (boost::filesystem::directory_iterator file(sDirName); file != boost::filesystem::directory_iterator(); ++file)
      {
         ...
         if (boost::filesystem::is_directory(current))
         {
            ...
            {
               String sErrorMessage;
               sErrorMessage.Format(PLATFORM_STRING("Could not delete the directory %s."), file->path().string());
   ...
   {
      if (sPath.StartsWith(PLATFORM_STRING("\\\\")))
         return true;
      else
         return false;
   }
   ...
   FileUtilities::Combine(const String &path1, const String &path2)
   {
      ...
      if (firstHalf.EndsWith(PLATFORM_STRING("\\")) || firstHalf.EndsWith(PLATFORM_STRING("/")))
         firstHalf = firstHalf.Mid(0, firstHalf.GetLength() -1);

      if (secondHalf.StartsWith(PLATFORM_STRING("\\")) || secondHalf.StartsWith(PLATFORM_STRING("/")))
   ...
   FileUtilities::IsValidUNCFolder(const String &sPath)
   {
      ...
      int shareStartPos = sPath.Find(PLATFORM_STRING("\\"), 3);
      if (shareStartPos < 0)
         return false;

      // We have at least \\server\

      int folderStartPos = sPath.Find(PLATFORM_STRING("\\"), shareStartPos + 1);
      if (folderStartPos < 0)
         return false;
