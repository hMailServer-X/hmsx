...
namespace HM
{
   ...
   MessageUtilities::~MessageUtilities()
   {
	...
	bool
	MessageUtilities::MoveToIMAPFolder(std::shared_ptr<Message> pMessage, long long iAccountID, const String &sFolderName, bool bAutoSubscribe, bool bSetByGlobalRule, long long &iResultAccount, long long &iResultFolder)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   ...
   //---------------------------------------------------------------------------()
	{
      ...
      if (!pFolder)
      {
         ...
         if (!pFolder)
         {
            ...
            sErrorMessage.Format(PLATFORM_STRING("Failed to locate folder. Account id: %d Path: %s"), iAccountID, sFolderName.c_str());
   ...
   MessageUtilities::CleanIMAPFolderRuleString(String &sIMAPFolder)
   {
      ...
      sYear.Format(PLATFORM_STRING("%.04d"), pTime.wYear);
      sMonth.Format(PLATFORM_STRING("%.02d"), pTime.wMonth);
      sDay.Format(PLATFORM_STRING("%.02d"), pTime.wDay);

      sIMAPFolder.ReplaceNoCase(PLATFORM_STRING("%YEAR%"), sYear);
      sIMAPFolder.ReplaceNoCase(PLATFORM_STRING("%MONTH%"), sMonth);
      sIMAPFolder.ReplaceNoCase(PLATFORM_STRING("%DAY%"), sDay);
   ...
   MessageUtilities::GetSendersIP(std::shared_ptr<Message> pMessage)
   {
      ...
      int iAddressStart = sReceivedValue.Find(PLATFORM_STRING("[")) +1;
      int iAddressEnd = sReceivedValue.Find(PLATFORM_STRING("]"), iAddressStart);
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      for(String header : headers)
      {
         ...
         if (hostName == PLATFORM_STRING("") || ipaddress == emptyAddress)
            continue;
   ...
   void MessageUtilitiesTester::Test()
   {
      ...
      if (hostName != PLATFORM_STRING("e113.en25.com"))
         throw;
