...
namespace HM
{
   ...
   TCPServer::InitAcceptor()
   {
      ...
      catch (boost::system::system_error error)
      {
         ...
         sErrorMessage.Format(PLATFORM_STRING("Failed to initialize local acceptor. Error: %s. Address: %s, Port: %d."), 
      ...
      const boost::system::error_code &error)
   {
      ...
      {
         String sMessage;
         sMessage.Format(PLATFORM_STRING("TCP - AcceptEx failed. Error code: %d, Message: %s"), error.value(), String(error.message()).c_str());
         LOG_DEBUG(sMessage);
      ...
      if (!error)
      {
         ...
         if (!allow)
         {
            ...
            String message;
            message.Format(PLATFORM_STRING("Client connection from %s was not accepted. Blocked either by IP range or by connection limit."), String(remoteAddress.ToString()).c_str());
            LOG_DEBUG(message);
            ...
            if (iBlockedIPHoldSeconds > 0)
            {
               ...
               message.Format(PLATFORM_STRING("Held connection from %s for %i seconds before dropping."), String(remoteAddress.ToString()).c_str(), iBlockedIPHoldSeconds);
               LOG_DEBUG(message);
            }
   ...
   TCPServer::FireOnAcceptEvent(std::shared_ptr<TCPConnection> connection, const IPAddress &remoteAddress, int port)
   {
      ...
      if (sScriptLanguage == PLATFORM_STRING("VBScript"))
         sEventCaller.Format(PLATFORM_STRING("OnClientConnect(HMAILSERVER_CLIENT)"));
