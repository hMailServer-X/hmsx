...
namespace HM
{
   ...
   DNSResolver::GetEmailServersRecursive_(const String &sDomainName, std::vector<HostNameAndIpAddress> &saFoundNames, int recursionLevel)
   {
      ...
      {
         String logMessage;
            logMessage.Format(PLATFORM_STRING("Failed to resolve email servers (MX lookup). Domain name: %s."), sDomainName.c_str());
      ...
      if (foundMxRecords.empty())
      {
         ...
         */
         ...
         {
            String logMessage;
               logMessage.Format(PLATFORM_STRING("Failed to resolve email servers (A lookup). Domain name: %s."), sDomainName.c_str());
            LOG_DEBUG(logMessage);
      ...
      String sLogMsg;
      sLogMsg.Format(PLATFORM_STRING("DNS - MX Result: %d IP addresses were found."), saFoundNames.size());
