   ...
   DNSResolverWinApi::Query(const String &query, int resourceType, std::vector<DNSRecord> &foundRecords)
   {
      ...
      if (!sCustomDNS.IsEmpty())
      {
         ...
         if (!pSrvList) {
            ...
            String sMessage;
            sMessage.Format(PLATFORM_STRING("Unable to allocate memory for DNS server list. Query: %s, Type: %d."), query, resourceType);
         ...
         if (pSrvList->AddrArray[0] == INADDR_NONE) {
            ...
            String sMessage;
            sMessage.Format(PLATFORM_STRING("Invalid DNSServer IP address. DNSServer IP: %hs."), sCustomDNS.c_str());
            ErrorManager::Instance()->ReportError(ErrorManager::Low, 4401, "DNSResolver::_Resolve", sMessage);
      ...
      if (nDnsStatus != 0)
      {
         ...
         {
            String sMessage;
            sMessage.Format(PLATFORM_STRING("DNS - Query failure. Query: %s, Type: %d, DnsQuery return value: %d."), query.c_str(), resourceType, nDnsStatus);
            LOG_TCPIP(sMessage);
            return false;
         }
