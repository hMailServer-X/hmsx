...
namespace HM
{
   ...
   TCPConnection::StartAsyncConnect_(const String &ip_adress, int port)
   {
      ...
      {
         String sMessage; 
            sMessage.Format(PLATFORM_STRING("Could not connect to %s on port %d since this would mean connecting to myself."), remote_ip_address_.c_str(), remote_port_);

         OnCouldNotConnect(sMessage);

         LOG_TCPIP(PLATFORM_STRING("TCPConnection - ") + sMessage);
   ...
   TCPConnection::AsyncHandshake()
   {
      ...
      if (enable_peer_verification)
      {
         ...
         if (!expected_remote_hostname_.IsEmpty())
         {
            ...
            {
               String errorMessage = Formatter::Format(PLATFORM_STRING("Failed to set verification callback for host {0}"), expected_remote_hostname_);
      ...
      if (error_code.value() != 0)
      {
         String error_message = Formatter::Format(PLATFORM_STRING("Failed to configure OpenSSL certificate verification: Mode: {0}"), verify_mode);
      ...
      else
      {
         ...
         if (!SSL_set_tlsext_host_name(ssl_socket_.native_handle(), expected_remote_hostname_.c_str()))
         {
            ...
            String error_message = Formatter::Format(PLATFORM_STRING("Failed to configure OpenSSL SNI. Expected remote host name: {0}."), expected_remote_hostname_);
   ...
   TCPConnection::AsyncHandshakeCompleted(const boost::system::error_code& error)
   {
      ...
      if (!error)
      {
         ...
         String sMessage;
         sMessage.Format(PLATFORM_STRING("TCPConnection - TLS/SSL handshake completed. Session Id: %d, Remote IP: %s, Version: %s, Cipher: %s, Bits: %d"), session_id_, SafeGetIPAddress().c_str(), String(cipher_info.GetVersion()).c_str(), String(cipher_info.GetName()).c_str(), cipher_info.GetBits());
         LOG_TCPIP(sMessage);
   ...
   TCPConnection::HandshakeFailed_(const boost::system::error_code& error)
   {
      ...
      String sMessage;
      sMessage.Format(PLATFORM_STRING("TCPConnection - TLS/SSL handshake failed. Session Id: %d, Remote IP: %s, Error code: %d, Message: %s"), session_id_, SafeGetIPAddress().c_str(), error.value(), String(error.message()).c_str());
      LOG_TCPIP(sMessage);
   ...
   TCPConnection::AsyncReadCompleted(const boost::system::error_code& error, size_t bytes_transferred)
   {
      ...
      if ((error.value() == 0 || error.value() == boost::asio::error::eof) && receive_binary_ && saEnabled && remote_port_ == saPort)
      {
         ...
         {
            String message;
            message.Format(PLATFORM_STRING("An error occured while parsing data. Data size: %d"), pBuffer->GetSize());
      ...
      else if (error.value() != 0)
      {
         ...
         String message;
         message.Format(PLATFORM_STRING("The read operation failed. Bytes transferred: %d"), bytes_transferred);
      ...
      else
      {
         ...
         else
         {
      ...
      #ifdef _DEBUG
            String sDebugOutput;
            sDebugOutput.Format(PLATFORM_STRING("RECEIVED: %s\r\n"), String(s).c_str());
            ...
            {
               String message;
               message.Format(PLATFORM_STRING("An error occured while parsing data. Data length: %d, Data: %s."), s.size(), String(s).c_str());
...
#ifdef _DEBUG
      String sDebugOutput;
      sDebugOutput.Format(PLATFORM_STRING("SENT: %s"), String(sTemp).c_str());
      OutputDebugString(sDebugOutput);
#endif
   ...
   TCPConnection::AsyncWriteCompleted(const boost::system::error_code& error, size_t bytes_transferred)
   {
      ...
      if (error.value() != 0)
      {
         ...
         String message;
         message.Format(PLATFORM_STRING("The write operation failed. Bytes transferred: %d"), bytes_transferred);
         ReportDebugMessage(message, error);
   ...
   TCPConnection::OnTimeout(std::weak_ptr<TCPConnection> connection, boost::system::error_code const& err)
   {
      ...
      String message;
      message.Format(PLATFORM_STRING("The client has timed out. Session: %d"), conn->GetSessionID());
      LOG_DEBUG(message);
   ...
   TCPConnection::ReportError(ErrorManager::eSeverity sev, int code, const String &context, const String &message, const boost::system::system_error &error)
   {
      ...
      formattedMessage.Format(PLATFORM_STRING("%s Remote IP: %s"), message.c_str(), SafeGetIPAddress().c_str());
   ...
   {
      String formattedMessage;
      formattedMessage.Format(PLATFORM_STRING("%s Remote IP: %s, Session: %d, Code: %d, Message: %s"), message.c_str(), SafeGetIPAddress().c_str(), GetSessionID(), error.value(), String(error.message()).c_str());
