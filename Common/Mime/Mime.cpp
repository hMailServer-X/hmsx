...
namespace HM
{
   ...
   void MimeHeader::SetBoundary(const char* pszBoundary/*=NULL*/)
   {
      ...
      {
         unsigned long long value = (unsigned long long)::time(NULL) ^ (unsigned long long)this;
   ...
   AnsiString MimeHeader::GetHeaderContents()
   {
      ...
      while (iterField != oFields.end())
      {
         ...
         sResponse += sHeaderField + PLATFORM_STRING(": ") + sHeaderValue + PLATFORM_STRING("\r\n");
   ...
   String MimeHeader::GetUnicodeHeaderContents()
   {
      ...
      while (iterField != oFields.end())
      {
         ...
         sResponse += String(sHeaderField) + PLATFORM_STRING(": ") + decodedValue + PLATFORM_STRING("\r\n");
...
#include <io.h>
      ...
      MimeBody::GenerateFileNameFromEncapsulatedSubject(bool unicode) const
   {
      ...
      if (!sFilename.IsEmpty())
      {
         ...
         // locally.
         sFilename.Replace(PLATFORM_STRING("\\"), PLATFORM_STRING(""));
         sFilename.Replace(PLATFORM_STRING("/"), PLATFORM_STRING(""));
         sFilename.Replace(PLATFORM_STRING(":"), PLATFORM_STRING(""));
      ...
      MimeBody::GetUnicodeFilename() const
   {
      ...
      if (sFilename.IsEmpty())
         sFilename.Format(PLATFORM_STRING("ATT-%d.dat"), part_index_);
      ...
      MimeBody::IsEncapsulatedRFC822Message() const
   {
      ...
      if (sMainType == PLATFORM_STRING("MESSAGE") && sSubType == PLATFORM_STRING("RFC822"))
         return true;
      else
         return false;
   }
      ...
      MimeBody::IsAttachment()  const
   {
      ...
      */
      ...
      if (sMainType.CompareNoCase(PLATFORM_STRING("multipart")) == 0)
      {
         // Multipart ...
         return false;
      }
   ...
   void MimeBody::Store(AnsiString &output, bool bIncludeHeader) const
   {
      ...
      for (BodyList::const_iterator it=bodies_.begin(); it!=bodies_.end(); it++)
      {
         ...
         AnsiString boundaryLine = Formatter::Format(PLATFORM_STRING("\r\n--{0}\r\n"), String(strBoundary));
      ...
      AnsiString endBoundaryLine = Formatter::Format(PLATFORM_STRING("\r\n--{0}--\r\n"), String(strBoundary));
