...
namespace HM
{
              ...
              Canonicalization::CanonicalizeMethod bodyMethod)
   {
      ...
      tagC.append(bodyMethod == Canonicalization::Simple ? PLATFORM_STRING("simple") : PLATFORM_STRING("relaxed"));
      String tagQ = "dns/txt";
   ...
   */
   ...
   DKIM::ValidateHeaderContents_(const DKIMParameters &signatureParams)
   {
      ...
      */
      ...
      if (!tagI.IsEmpty())
      {
         ...
         {
            String sMessage;
            sMessage.Format(PLATFORM_STRING("DKIM: Header in message incomplete. Tag I mismatch (%s - %s). Aborting DKIM test."), String(tagD).c_str(), String(tagIDomain).c_str());
            LOG_DEBUG(sMessage);
         ...
         */
         ...
         if (!tagI.IsEmpty())
         {
            ...
            if (tagIDomain.CompareNoCase(tagD) != 0)
            {
               ...
               String sMessage;
               sMessage.Format(PLATFORM_STRING("DKIM: Header in message incomplete. Tag I mismatch (%s - %s). Aborting."), String(tagD).c_str(), String(tagIDomain).c_str());
               LOG_DEBUG(sMessage);
   ...
   DKIM::BuildSignatureHeader_(const String &tagA, const String &tagD, const String &tagS, const String &tagC, const String &tagQ, const String &fieldList, const String &bodyHash, const String &signatureString)
   {
      ...
      {
         headerValue.Format(PLATFORM_STRING("v=1; a=%s; d=%s; s=%s;\r\n")
            PLATFORM_STRING("\tc=%s; q=%s; h=%s;\r\n")
            PLATFORM_STRING("\tbh=%s;\r\n")
            PLATFORM_STRING("\tb="), tagA.c_str(), tagD.c_str(), tagS.c_str(), tagC.c_str(), tagQ.c_str(), String(fieldList).c_str(), bodyHash.c_str());
      }
      else
      {
         ...
         headerValue.Format(PLATFORM_STRING("v=1; a=%s; d=%s; s=%s;\r\n")
            PLATFORM_STRING("\tc=%s; q=%s; h=%s;\r\n")
            PLATFORM_STRING("\tbh=%s;\r\n")
            PLATFORM_STRING("\tb=%s"), tagA.c_str(), tagD.c_str(), tagS.c_str(), tagC.c_str(), tagQ.c_str(), String(fieldList).c_str(), bodyHash.c_str(), splitSignatureString.c_str());
      }
