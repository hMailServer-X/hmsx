...
namespace HM
{
   ...
   SpamAssassinClient::OnConnected()
   {
	  ...
	  String sConLen;
	  sConLen.Format(PLATFORM_STRING("Content-length: %d\r\n"), message_size_);
   ...
   {
      String logMessage;
      logMessage.Format(PLATFORM_STRING("Failed to connect to SpamAssassin. Session %d"), GetSessionID());
   ...
   {
      String logMessage;
      logMessage.Format(PLATFORM_STRING("Sending message to SpamAssassin. Session %d, File: %s"), GetSessionID(), sFilename.c_str());
      LOG_DEBUG(logMessage);
      ...
      {
         String sErrorMsg;
         sErrorMsg.Format(PLATFORM_STRING("Could not send file %s via socket since it does not exist."), sFilename.c_str());
   ...
   SpamAssassinClient::ParseData(std::shared_ptr<ByteBuffer> pBuf)
   {
      ...
      {
         String logMessage;
         logMessage.Format(PLATFORM_STRING("Parsing response from SpamAssassin. Session %d"), GetSessionID());
         LOG_DEBUG(logMessage);
   ...
   SpamAssassinClient::FinishTesting_()
   {
     ...
     {
		 String logMessage;
		 logMessage.Format(PLATFORM_STRING("SA: Temp file size did not match what Spamd reported! (temp: %d, spamd: %d). Reverting to original message file."),FileUtilities::FileSize(sTempFile),spam_dsize_);
         LOG_DEBUG(logMessage);
	  }
   ...
   */
   ...
   {
      String errorMessage;
      errorMessage.Format(PLATFORM_STRING("There was a communication error with SpamAssassin. ")
                          PLATFORM_STRING("hMailServer tried to retrieve data from SpamAssassin but the connection ")
                          PLATFORM_STRING("to SpamAssassin was lost. The WinSock error code is %d. Enable debug ")
                          PLATFORM_STRING("logging to retrieve more information regarding this problem. ")
                          PLATFORM_STRING("The problem could be that SpamAssassin is malfunctioning."), errorCode);
