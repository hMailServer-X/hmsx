...
namespace HM
{
   ...
   Logger::LogSMTPConversation(int iSessionID, const String &sRemoteHost, const String &sMessage, bool bClient)
   {
      ...
      if (bClient)
         sData.Format(PLATFORM_STRING("\"SMTPC\"\t%d\t%d\t\"%s\"\t\"%s\"\t\"%s\"\r\n"), lThread, iSessionID, sTime.c_str(), sRemoteHost.c_str(), CleanLogMessage_(sMessage).c_str());
      else
         sData.Format(PLATFORM_STRING("\"SMTPD\"\t%d\t%d\t\"%s\"\t\"%s\"\t\"%s\"\r\n"), lThread, iSessionID, sTime.c_str(), sRemoteHost.c_str(), CleanLogMessage_(sMessage).c_str());
   ...
   Logger::LogPOP3Conversation(int iSessionID, const String &sRemoteHost, const String &sMessage, bool bClient)
   {
      ...
      if (bClient)
         sData.Format(PLATFORM_STRING("\"POP3C\"\t%d\t%d\t\"%s\"\t\"%s\"\t\"%s\"\r\n"), lThread, iSessionID, sTime.c_str(), sRemoteHost, CleanLogMessage_(sMessage).c_str());
      else
         sData.Format(PLATFORM_STRING("\"POP3D\"\t%d\t%d\t\"%s\"\t\"%s\"\t\"%s\"\r\n"), lThread, iSessionID, sTime.c_str(), sRemoteHost, CleanLogMessage_(sMessage).c_str());
   ...
   Logger::LogIMAPConversation(int iSessionID, const String &sRemoteHost, const String &sMessage)
   {
      ...
      String sData;
      sData.Format(PLATFORM_STRING("\"IMAPD\"\t%d\t%d\t\"%s\"\t\"%s\"\t\"%s\"\r\n"), lThread, iSessionID, sTime.c_str(), sRemoteHost.c_str(), CleanLogMessage_(sMessage).c_str());
   ...
   Logger::LogDebug(const String &sMessage)
   {
      ...
      String sData;
      sData.Format(PLATFORM_STRING("\"DEBUG\"\t%d\t\"%s\"\t\"%s\"\r\n"), lThread, sTime.c_str(), CleanLogMessage_(sMessage).c_str());
   ...
   Logger::LogError(const String &sMessage)
   {
      ...
      String sData;
      sData.Format(PLATFORM_STRING("\"ERROR\"\t%d\t\"%s\"\t\"%s\"\r\n"), lThread, sTime.c_str(), CleanLogMessage_(sMessage).c_str());
   ...
   Logger::LogTCPIP(const String &sMessage)
   {
      ...
      String sData;
      sData.Format(PLATFORM_STRING("\"TCPIP\"\t%d\t\"%s\"\t\"%s\"\r\n"), lThread, sTime.c_str(), CleanLogMessage_(sMessage).c_str());
   ...
   Logger::LogEvent(const String &sMessage)
   {
      ...
      String sData;
      sData.Format(PLATFORM_STRING("%d\t\"%s\"\t\"%s\"\r\n"), lThread, sTime.c_str(), CleanLogMessage_(sMessage).c_str());
   ...
   Logger::CleanLogMessage_(const String &message)
   {
      ...
      result.Replace(PLATFORM_STRING("\r\n"), PLATFORM_STRING("[nl]"));
      return result;
   }
   ...
   Logger::GetCurrentLogFileName(LogType lt) 
   {
      ...
      {
      case Normal:
         sFilename.Format(PLATFORM_STRING("%s\\hmailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case Error:
         sFilename.Format(PLATFORM_STRING("%s\\ERROR_hmailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case AWStats:
         sFilename.Format(PLATFORM_STRING("%s\\hmailserver_awstats.log"), log_dir_.c_str());
         break;
      case Backup:
         sFilename.Format(PLATFORM_STRING("%s\\hmailserver_backup.log"), log_dir_.c_str());
         break;
      case Events:
         sFilename.Format(PLATFORM_STRING("%s\\hmailserver_events.log"), log_dir_.c_str());
         break;
      case IMAP:
         if (sep_svc_logs_) 
            sFilename.Format(PLATFORM_STRING("%s\\hmailserver_IMAP_%s.log"), log_dir_.c_str(), theTime.c_str());
         else
            sFilename.Format(PLATFORM_STRING("%s\\hmailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case POP3:
         if (sep_svc_logs_) 
            sFilename.Format(PLATFORM_STRING("%s\\hmailserver_POP3_%s.log"), log_dir_.c_str(), theTime.c_str());
         else
            sFilename.Format(PLATFORM_STRING("%s\\hmailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      case SMTP:
         if (sep_svc_logs_) 
            sFilename.Format(PLATFORM_STRING("%s\\hmailserver_SMTP_%s.log"), log_dir_.c_str(), theTime.c_str());
         else
            sFilename.Format(PLATFORM_STRING("%s\\hmailserver_%s.log"), log_dir_.c_str(), theTime.c_str());
         break;
      }
   ...
   Logger::LogBackup(const String &sData)
   {
      ...
      sLogMessage.Format(PLATFORM_STRING("%s\t%s\r\n"), sTime.c_str(), sData.c_str());
