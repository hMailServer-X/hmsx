...
namespace HM
{
   ...
   */
   ...
   ACLManager::GetPermissionForFolder(long long iAccountID, std::shared_ptr<IMAPFolder> pFolder)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Input:
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      while (pCheckFolder && maxRecursions > 0)
      {
         ...
         long long iParentFolderID = pCheckFolder->GetParentFolderID();
   ...
   std::shared_ptr<ACLPermission> 
   ACLManager::GetPermissionForAccount_(std::shared_ptr<ACLPermissions> pPermissions, long long iAccountID)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      std::list<std::pair<long long, std::shared_ptr<ACLPermission> > > listGroupPermissions;
      for (; iter != iterEnd; iter++)
      {
         std::shared_ptr<ACLPermission> pPermission = (*iter);
      ...
      {
         long long iGroupID = (*iterGroup).first;
         ...
         {
            String sMessage;
            sMessage.Format(PLATFORM_STRING("The group referenced by ACL ID %I64d (Group ID %I64d, Folder ID %I64d) does not exist. "), 
