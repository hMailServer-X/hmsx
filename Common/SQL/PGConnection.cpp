...
namespace HM
{
   ...
   PGConnection::Connect(String &sErrorMessage)
   {
      try
      {
         ...
         sConnectionString.Format(PLATFORM_STRING("host='%s' port='%d' user='%s' password='%s'"), sServer.c_str(), lDBPort, sUsername.c_str(), sPassword.c_str());
   ...
   PGConnection::TryExecute(const SQLCommand &command, String &sErrorMessage, long long *iInsertID, int iIgnoreErrors) 
   {
      ...
      try
      {
         ...
         bool bIgnoreErrors = SQL.Find(PLATFORM_STRING("[IGNORE-ERRORS]")) >= 0;
   ...
   PGConnection::CheckError(PGresult *pResult, const String &sAdditionalInfo, String &sOutputErrorMessage) const
   {
      try
      {
         ...
         sErrorMessage.Format(PLATFORM_STRING("Postgres: %s (Additional info: %s)"), sErrorMsg.c_str(), sAdditionalInfo);
   ...
   {
      sInput.Replace(PLATFORM_STRING("'"), PLATFORM_STRING("''"));
      sInput.Replace(PLATFORM_STRING("\\"), PLATFORM_STRING("\\\\"));
   }
