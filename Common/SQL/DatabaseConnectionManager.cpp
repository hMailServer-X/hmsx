...
namespace HM
{
   ...
   bool 
   DatabaseConnectionManager::Execute(const SQLStatement &statement, long long *iInsertID, int iIgnoreErrors, String &sErrorMessage)
   {
      return Execute(statement.GetCommand(), iInsertID, iIgnoreErrors, sErrorMessage);
   }

   bool 
   DatabaseConnectionManager::Execute(const SQLCommand &command, long long *iInsertID, int iIgnoreErrors, String &sErrorMessage)
   {
