...
#define XHMAILSERVER_LOOPCOUNT PLATFORM_STRING("X-hMailServer-LoopCount")
#define AUTOSUBMITTED_HEADER PLATFORM_STRING("Auto-Submitted")
...
namespace HM
{
   ...
   MessageData::LoadFromMessage(const String &fileName, std::shared_ptr<Message> pMessage)
   {
      ...
      catch (...)
      {
         try
         {
            ...
            sErrorMessage.Format(PLATFORM_STRING("An unknown error occurred while loading message. File: %s. Backuped to: %s"), message_file_name_.c_str(), sMessageBackupPath.c_str());
   ...
   MessageData::SetBody(const String &sBody)
   {
      ...
      if (sModifiedBody.Right(2) != PLATFORM_STRING("\r\n"))
      {
         // Add carriage return.
         sModifiedBody += "\r\n";
      }
   ...
   MessageData::CreatePart(const String &sContentType)
   {
      ...
      if (sContentType.CompareNoCase(PLATFORM_STRING("text/plain")) == 0)
      {
         assert (textPart == 0);
      ...
      else if (sContentType.CompareNoCase(PLATFORM_STRING("text/html")) == 0)
      {
         assert (htmlPart == 0);
   ...
   MessageData::GetBodyTextPlainPart() const
   {
      ...
      if (part_type == PLATFORM_STRING("text/plain"))
         return mime_mail_;
   ...
   MessageData::GetBodyTextHtmlPart() const
   {
      ...
      if (part_type == PLATFORM_STRING("text/html"))
         return mime_mail_;
   ...
   MessageData::GetViewBodyPart_(int recursion_level, std::shared_ptr<MimeBody> source, const String &requested_content_type) const
   {
      ...
      if (main_type.CompareNoCase(PLATFORM_STRING("multipart")) == 0)
      {
   ...
   MessageData::GenerateMessageID()
   {
      ...
      String sMsgID;
      sMsgID.Format(PLATFORM_STRING("<%s@%s>"), sGUID, Utilities::ComputerName().c_str());
   ...
   {
      return sContentType.CompareNoCase(PLATFORM_STRING("text/plain")) == 0;
   }

   bool 
   MessageData::IsHTMLType(const String &sContentType)
   {
      return sContentType.CompareNoCase(PLATFORM_STRING("text/html")) == 0;
   }
   ...
   {
	   SetFieldValue(AUTOSUBMITTED_HEADER, PLATFORM_STRING("auto-replied"));
   }

   void
   MessageData::SetAutoGenerated()
   {
	   SetFieldValue(AUTOSUBMITTED_HEADER, PLATFORM_STRING("auto-generated"));
   }
   ...
   MessageData::IsAutoSubmitted()
   {
		...
		if (autoSubmitted.CompareNoCase(PLATFORM_STRING("no")) == 0)
			return false;
