...
namespace HM
{
   RuleActions::RuleActions(long long iRuleID) :
      rule_id_(iRuleID)
   {
   }
   ...
   {
      String sSQL;
      sSQL.Format(PLATFORM_STRING("select * from hm_rule_actions where actionruleid = %I64d order by actionsortorder asc"), rule_id_);
