...
namespace HM
{
   ...
   public:
      ...
      std::shared_ptr<T> GetItemByDBID(unsigned long long DBID) const;
      std::shared_ptr<T> GetItemByDBID(unsigned long long DBID, int &foundIndex) const;
      std::shared_ptr<T> GetItemByName(const String &sName) const;

      bool DeleteItemByDBID(long long iDBID);
         ...
         for (int i = 0; i < pCollNode->GetChildCount(); i++)
         {
            ...
            if (PreSaveObject(pItem, pChildNode))
            {
               ...
               if (!P::SaveObject(pItem, result, PersistenceModeRestore))
               {
                  // Handle failure..
                  String message;
                  message.Format(PLATFORM_STRING("Failed to save object %s. Error: %s"), pItem->GetName().c_str(), result);
   ...
   template <class T, class P> 
   std::shared_ptr<T> Collection<T,P>::GetItemByDBID(unsigned long long DBID) const
   {
      boost::lock_guard<boost::recursive_mutex> guard(_mutex);
   ...
   template <class T, class P> 
   std::shared_ptr<T> Collection<T,P>::GetItemByDBID(unsigned long long DBID, int &foundIndex) const
   {
      boost::lock_guard<boost::recursive_mutex> guard(_mutex);
   ...
   template <class T, class P> 
   bool Collection<T,P>::DeleteItemByDBID(long long DBID)
   {
      boost::lock_guard<boost::recursive_mutex> guard(_mutex);
