...
namespace HM
{
   ...
   IMAPFolders::IMAPFolders(long long iAccountID, long long iParentFolderID) :
   ...
   IMAPFolders::Refresh()
   {
      ...
      std::vector<std::pair<long long, std::shared_ptr<IMAPFolder> > > vecIMAPFolders;

      if (!pRS->IsEOF())
      {
         long long iFolderID = 0;
         long long iParentID = 0;
         String sFolderName;
         ...
         while (vecIMAPFolders.size() > 0)
         {
            ...
            {
               long long iFolderParentID = (*iterFolder).first;

               if (iFolderParentID == (long long) -1)
               {
            ...
            if (lPanicLevel > 100000)
            {
			   ...
			   sMessage.Format(PLATFORM_STRING("Unable to retrieve folder list for account. 100K+ loops trying to sort. Account: %I64d, Parent: %I64d"), account_id_, parent_folder_id_);
   ...
   IMAPFolders::RemoveFolder(std::shared_ptr<IMAPFolder> pFolderToRemove)
   {
      ...
      long long lRemoveFolderID = pFolderToRemove->GetID();
   ...
   std::shared_ptr<IMAPFolder> 
   IMAPFolders::GetItemByDBIDRecursive(long long lFolderID)
   {
      boost::lock_guard<boost::recursive_mutex> guard(_mutex);
