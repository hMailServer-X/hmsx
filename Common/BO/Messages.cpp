...
namespace HM
{
   ...
   Messages::Messages(long long iAccountID, long long iFolderID) :
   ...
   Messages::Refresh(bool update_recent_flags)
   {
      ...
      else
      {
         ...
         sSQL += PLATFORM_STRING(" messageaccountid = @MESSAGEACCOUNTID ");
      ...
      if (folder_id_ != -1)
      {
         sSQL.AppendFormat(PLATFORM_STRING(" and messagefolderid = @MESSAGEFOLDERID "));
      ...
      {
         sSQL.AppendFormat(PLATFORM_STRING(" and messageuid > @MESSAGEUID "));
   ...
   void 
   Messages::Remove(long long iDBID)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
