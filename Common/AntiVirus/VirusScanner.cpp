   ...
   */
   ...
   VirusScanner::Scan(std::shared_ptr<Message> pMessage, String &virusName)
   {
      ...
      while (iter != oList.end())
      {
         ...
         sLongFilename.Format(PLATFORM_STRING("%s\\%s.tmp"), IniFileSettings::Instance()->GetTempDirectory().c_str(), GUIDCreator::GetGUID().c_str());
   ...
   VirusScanner::ReportVirusFound(std::shared_ptr<Message> pMessage)
   {
      ...
      String sMessage;
      sMessage.Format(PLATFORM_STRING("Virus found in message from %s. Taking actions"), pMsgData->GetFrom().c_str());
   ...
   VirusScanner::BlockAttachments(std::shared_ptr<Message> message)
   {
      ...
      for (auto attachment : pAttachments->GetVector())
      {
         ...
         if (matching_wildcard != blocked_attachment_wildcards.end())
         {
            ...
            // Replace macros.
            updated_message_body.Replace(PLATFORM_STRING("%MACRO_FILE%"), attachment->GetFileName());
