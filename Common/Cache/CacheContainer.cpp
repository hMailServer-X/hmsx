...
namespace HM
{
   ...
   CacheContainer::GetAccount(long long iID)
   {
      CacheReaderWithDbFallback<Account, PersistentAccount, long long> reader;
   ...
   {
#ifdef _DEBUG
      if (sName.Contains(PLATFORM_STRING("@")))
      {
         assert(0);
      }
#endif
   ...
   std::shared_ptr<const Domain> 
   CacheContainer::GetDomain(long long iID)
   {
      CacheReaderWithDbFallback<Domain, PersistentDomain, long long> reader;
      return reader.GetObject(iID);
   }
   ...
   std::shared_ptr<const Alias> 
   CacheContainer::GetAlias(long long iID)
   {
      CacheReaderWithDbFallback<Alias, PersistentAlias, long long> reader;
      return reader.GetObject(iID);
   }
   ...
   std::shared_ptr<const DistributionList> 
   CacheContainer::GetDistributionList(long long iID)
   {
      CacheReaderWithDbFallback<DistributionList, PersistentDistributionList, long long> reader;
      return reader.GetObject(iID);
   }
