...
namespace HM
{
   ...
   private:
      ...
      typedef multi_index_container<
         ...
         indexed_by<
            hashed_unique<
            tag<id>, BOOST_MULTI_INDEX_MEMBER(CachedObject<typename T>, long long, id_)>,
            hashed_non_unique<
   ...
   template <class T> 
   void 
   Cache<T>::RemoveObject(long long iID)
   {
      boost::lock_guard<boost::recursive_mutex> guard(_mutex);
   ...
   template <class T> 
   std::shared_ptr<T> 
   Cache<T>::GetObject(long long iID)
   {
      boost::lock_guard<boost::recursive_mutex> guard(_mutex);
