...
namespace HM
{
   ...
   POP3ClientConnection::InternalParseData(const String &sRequest)
   {
      ...
      if (sAccountName.StartsWith(PLATFORM_STRING("ETRN")))
      {
   ...
   POP3ClientConnection::SendUserName_()
   {
      ...
      String sResponse;
      sResponse.Format(PLATFORM_STRING("USER %s"), account_->GetUsername().c_str());
   ...
   POP3ClientConnection::SendCAPA_()
   {
      // We have connected successfully.
      // Time to send the username.
      EnqueueWrite_(PLATFORM_STRING("CAPA"));
   ...
   {
      if (!CommandIsSuccessfull_(sData) || !sData.Contains(PLATFORM_STRING("STLS")))
      {
   ...
   POP3ClientConnection::ParseUsernameSent_(const String &sData)
   {
      if (CommandIsSuccessfull_(sData))
      {
         ...
         String sResponse;
         sResponse.Format(PLATFORM_STRING("PASS %s"), account_->GetPassword().c_str());
   ...
   POP3ClientConnection::ParsePasswordSent_(const String &sData)
   {
      if (CommandIsSuccessfull_(sData))
      {
         ...
         // Time to send the username.
         String sResponse;
         sResponse.Format(PLATFORM_STRING("UIDL"));
   ...
   POP3ClientConnection::ParseUIDLResponse_(const String &sData)
   {
      if (CommandIsSuccessfull_(sData))
      {
         ...
         while (iter != vecLines.end())
         {
            ...
            if (sLine == PLATFORM_STRING("."))
               break;

            int iSpacePos = sLine.Find(PLATFORM_STRING(" "));
   ...
   POP3ClientConnection::RequestNextMessage_()
   {
      while (cur_message_ != uidlresponse_.end())
      {
         ...
         else
         {
            ...
            String sResponse;
            sResponse.Format(PLATFORM_STRING("RETR %d"), iMessageIdx);
   ...
   {
      String sResponse;
      sResponse.Format(PLATFORM_STRING("QUIT"));
   ...
   {
      if (sData.Mid(0,3).CompareNoCase(PLATFORM_STRING("+OK")) == 0)
         return true;
      else
         return false;
   }
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      String sHeader;
      sHeader.Format(PLATFORM_STRING("X-hMailServer-ExternalAccount: %s\r\n"), account_->GetName().c_str());
   ...
   */
   ...
   POP3ClientConnection::MessageCleanup_()
   {
      ...
      // Delete the message.
      sResponse.Format(PLATFORM_STRING("DELE %d"), iIndex);
      EnqueueWrite_(sResponse);
   ...
   POP3ClientConnection::LogPOP3String_(const String &sLogString, bool bSent)
   {
      ...
      sTemp.TrimRight(PLATFORM_STRING("\r\n"));
