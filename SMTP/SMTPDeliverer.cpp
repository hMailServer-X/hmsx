...
namespace HM
{
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      long long messageID = pMessage->GetID();
      ...
      if (deleteMessageNow)
      {
         ...
         {
            String errorMessage;
            errorMessage.Format(PLATFORM_STRING("Attempting to delete message %I64d even though it's been delivered to user account %I64d ."), messageID, pMessage->GetAccountID());
      ...
      String logText;
      logText.Format(PLATFORM_STRING("SMTPDeliverer - Message %I64d: Message delivery thread completed."), messageID);
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      {
         String sMessage;
         sMessage.Format(PLATFORM_STRING("Did not submit bounce message for message %I64d from %s since rule loop count was reached or Auto-Submitted header."), pOrigMessage->GetID(), String(pOrigMessage->GetFromAddress()).c_str());
      ...
      sMessageUndeliverable.Format(PLATFORM_STRING("%s"), Configuration::Instance()->GetServerMessages()->GetMessage("MESSAGE_UNDELIVERABLE").c_str());
      String sErrMsg = Configuration::Instance()->GetServerMessages()->GetMessage("SEND_FAILED_NOTIFICATION");

      sErrMsg.Replace(PLATFORM_STRING("%MACRO_SENT%"), pMsgData->GetSentTime());
      sErrMsg.Replace(PLATFORM_STRING("%MACRO_SUBJECT%"), pMsgData->GetSubject());

      // Ability to include original headers in undeliverable message
      // http://www.hmailserver.com/forum/viewtopic.php?f=2&t=19635
      sErrMsg.Replace(PLATFORM_STRING("%MACRO_ORIGINAL_HEADER%"), pMsgData->GetHeader());      

      String sCollectedErrors;
      for (unsigned int i=0;i<saErrorMessages.size();i++)
         sCollectedErrors += saErrorMessages[i];

      sErrMsg.Replace(PLATFORM_STRING("%MACRO_RECIPIENTS%"), sCollectedErrors);
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (ruleResult.GetDeleteEmail())
      {
         ...
         String sMessage;
         sMessage.Format(PLATFORM_STRING("SMTPDeliverer - Message %I64d: ")
            PLATFORM_STRING("Message will be deleted. Action triggered by a global rule (%s). "),
            pMessage->GetID(), 
