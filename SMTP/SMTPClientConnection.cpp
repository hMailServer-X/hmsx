...
namespace HM
{
   ...
   SMTPClientConnection::LogReceivedResponse_(const String &response)
   {
      ...
      log_data.TrimRight(PLATFORM_STRING("\r\n"));
   ...
   SMTPClientConnection::EnqueueWrite_(const String &sData)
   {
      ...
      if (current_state_ == PASSWORDSENT)
         last_sent_data_ = PLATFORM_STRING("<Password removed>");
      else
      {
   ...
   SMTPClientConnection::UpdateRecipientWithError_(int iErrorCode, const AnsiString &sResponse, std::shared_ptr<MessageRecipient> pRecipient, bool bPreConnectError)
   {
      ...
      if (bPreConnectError)
      {
         sData.Format(PLATFORM_STRING("   Error Type: SMTP\r\n")
            PLATFORM_STRING("   Connection to recipients server failed.\r\n")
            PLATFORM_STRING("   Error: %s\r\n"),
            String(sResponse).c_str());
      }
      else
      {
         sData.Format(PLATFORM_STRING("   Error Type: SMTP\r\n")
            PLATFORM_STRING("   Remote server (%s) issued an error.\r\n")
            PLATFORM_STRING("   hMailServer sent: %s\r\n")
            PLATFORM_STRING("   Remote server replied: %s\r\n"),
