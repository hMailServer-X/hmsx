...
{ 
   ...
   LocalDelivery::DeliverToLocalAccount_(std::shared_ptr<const Account> account, size_t iNoOfRecipients, std::vector<String> &saErrorMessages, const String &sOriginalAddress, bool &messageReused)
   {
      ...
      {
         String errorMessage;
         errorMessage.Format(PLATFORM_STRING("Unable to create account-level message of %I64d for account %s."), original_message_->GetID(), String(account->GetAddress()).c_str());
   ...
   */
   ...
   LocalDelivery::LocalDeliveryPreProcess_(std::shared_ptr<const Account> account, std::shared_ptr<Message> accountLevelMessage, const String &sOriginalAddress, std::vector<String> &saErrorMessages)
   {
      ...
      //
      long long iDestinationFolderID = accountLevelMessage->GetFolderID();
      long long iDestinationAccountID = account->GetID();
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (!pCheckAccount->SpaceAvailable(original_message_->GetSize()))
      {
         ...
         long long currentSize = AccountSizeCache::Instance()->GetSize(pCheckAccount->GetID()) + original_message_->GetSize();

         String sMessage;
         sMessage.Format(PLATFORM_STRING("SMTPDeliverer - Message %I64d: The message was not delivered to %s. ")
            PLATFORM_STRING("Delivery to this account was cancelled since the account inbox is full. Max size: %d MB, Current size (including cancelled message): %d MB"), 
   ...
   LocalDelivery::CreateAccountLevelMessage_(std::shared_ptr<Message> pOriginalMessage, std::shared_ptr<const Account> pRecipientAccount, bool reuseMessage, const String &sOriginalAddress)
   {
      ...
      if (reuseMessage)
      {
         ...
         long long  inboxID = CacheContainer::Instance()->GetInboxIDCache().GetUserInboxFolder(pRecipientAccount->GetID());
         if (inboxID == 0)
         {
