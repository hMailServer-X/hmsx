...
namespace HM
{
   ...
   {
      if (sFirstWord == PLATFORM_STRING("HELO"))
         return SMTP_COMMAND_HELO;
      else if (sFirstWord == PLATFORM_STRING("HELP"))
         return SMTP_COMMAND_HELP;
      else if (sFirstWord == PLATFORM_STRING("QUIT"))
         return SMTP_COMMAND_QUIT;
      else if (sFirstWord == PLATFORM_STRING("EHLO"))
         return SMTP_COMMAND_EHLO;
      else if (sFirstWord == PLATFORM_STRING("AUTH"))
         return SMTP_COMMAND_AUTH;
      else if (sFirstWord == PLATFORM_STRING("MAIL"))
         return SMTP_COMMAND_MAIL;
      else if (sFirstWord == PLATFORM_STRING("RCPT"))
         return SMTP_COMMAND_RCPT;
      else if (sFirstWord == PLATFORM_STRING("TURN"))
         return SMTP_COMMAND_TURN;
      else if (sFirstWord == PLATFORM_STRING("VRFY"))
         return SMTP_COMMAND_VRFY;
      else if (sFirstWord == PLATFORM_STRING("DATA"))
         return SMTP_COMMAND_DATA;
      else if (sFirstWord == PLATFORM_STRING("RSET"))
         return SMTP_COMMAND_RSET;
      else if (sFirstWord == PLATFORM_STRING("NOOP"))
         return SMTP_COMMAND_NOOP;
      else if (sFirstWord == PLATFORM_STRING("ETRN"))
         return SMTP_COMMAND_ETRN;
      else if (sFirstWord == PLATFORM_STRING("STARTTLS"))
      ...
      if (Logger::Instance()->GetLogSMTP())
      {
         ...
         if (current_state_ == HEADER && boost::regex_match(sLogData, matches, expression))
         {
            if (matches.size() > 0)
            {
               ...
               int iSecondTab = sAuthentication.Find(PLATFORM_STRING("\t"), 1);
               if (iSecondTab > 0)
               {
         ...
         default:
            {
               ErrorManager::Instance()->ReportError(ErrorManager::Medium, 5500, "SMTPConnection::InternalParseData", 
                  Formatter::Format(PLATFORM_STRING("Received unexpected string data: {0}"), sRequest));
            }
      }
   ...
   SMTPConnection::ProtocolMAIL_(const String &Request)
   {
      ...
      if (!Request.StartsWith(PLATFORM_STRING("MAIL FROM:")))
      {
         SendErrorResponse_(550, "Invalid syntax. Syntax should be MAIL FROM:<mailbox@domain>[crlf]");
         return;
      }
      ...
      while (iterParam != vecParams.end())
      {
         ...
         if (parameter.Left(4).CompareNoCase(PLATFORM_STRING("SIZE")) == 0)
            iEstimatedMessageSize = _ttoi(parameter.Mid(5));
         else if (parameter.Left(4).CompareNoCase(PLATFORM_STRING("AUTH")) == 0)
          ...
          iEstimatedMessageSize / 1024 > max_message_size_kb_)
      {
         ...
         String sMessage;
         sMessage.Format(PLATFORM_STRING("552 Message size exceeds fixed maximum message size. Size: %d KB, Max size: %d KB"), 
               iEstimatedMessageSize / 1024, max_message_size_kb_);
   ...
   SMTPConnection::ProtocolRCPT_(const String &Request)
   {
      ...
      if (!Request.StartsWith(PLATFORM_STRING("RCPT TO:")))
      {
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (deleteThreshold > 0 && iTotalSpamScore >= deleteThreshold)
      {
         ...
         String sLogMessage;
         sLogMessage.Format(PLATFORM_STRING("hMailServer SpamProtection rejected RCPT (Sender: %s, IP:%s, Reason: %s)"), sFromAddress.c_str(), String(GetIPAddressString()).c_str(), messageText.c_str());
   ...
   //---------------------------------------------------------------------------()
   {
      ...
      if (!transmission_buffer_->GetTransmissionEnded())
      {
         ...
         {
            sLogData.Format(PLATFORM_STRING("Size: %d KB, Max size: %d KB - DROP!!"), 
            iBufSizeKB, iMaxSizeDrop);
            LOG_SMTP(GetSessionID(), GetIPAddressString(), sLogData);      
            String sMessage;
            sMessage.Format(PLATFORM_STRING("552 Message size exceeds the drop maximum message size. Size: %d KB, Max size: %d KB - DROP!"), 
                iBufSizeKB, iMaxSizeDrop);
            EnqueueWrite_(sMessage);
   ...
   SMTPConnection::HandleSMTPFinalizationTaskCompleted_()
   {
      ...
      if (OnPreAcceptTransfer_())
      {
         ...
         if (PersistentMessage::SaveObject(current_message_))
         {
            ...
            // start a new message.
            String sResponse;
            sResponse.Format(PLATFORM_STRING("250 Queued (%.3f seconds)"), dTCDiff);
       ...
       SMTPConnection::OnPreAcceptTransfer_()
   {
       ...
       {
           String sMessage;
           sMessage.Format(PLATFORM_STRING("554 Rejected - Message size exceeds fixed maximum message size. Size: %d KB, Max size: %d KB"),
       ...
       if (!Configuration::Instance()->GetSMTPConfiguration()->GetAllowIncorrectLineEndings())
       {
           ...
           {
               String sMessage;
               sMessage.Format(PLATFORM_STRING("554 Rejected - Message containing bare LF's."));
   ...
   SMTPConnection::SendEHLOKeywords_()
   {
      ...
      // Append size keyword
      {
         ...
         if (iMaxSize > 0)
            sSizeKeyword.Format(PLATFORM_STRING("\r\n250-SIZE %d"), iMaxSize);
         else
            sSizeKeyword.Format(PLATFORM_STRING("\r\n250-SIZE"));
         sData += sSizeKeyword;
      }
   ...
   SMTPConnection::EnqueueWrite_(const String &sData)
   {
      if (Logger::Instance()->GetLogSMTP())
      {
         ...
         sLogData.TrimRight(PLATFORM_STRING("\r\n"));
   ...
   {
      int iFirstSpace = sRequest.Find(PLATFORM_STRING(" "));
   ...
   SMTPConnection::ProtocolAUTH_(const String &sRequest)
   {
      ...
      if (sAuthenticationType == PLATFORM_STRING("LOGIN"))
      {
      ...
      else if (sAuthenticationType == PLATFORM_STRING("PLAIN") && 
...
// WE SHOULD ADD SOME LOGGING HERE
      ...
      if (route && route->GetName() == sETRNDomain2)
      {
         ...
         if (pRoute)
         {
            long long iRouteID = pRoute->GetID();
   ...
   SMTPConnection::SendErrorResponse_(int iErrorCode, const String &sResponse)
   {
      ...
      String sData;
      sData.Format(PLATFORM_STRING("%d %s"), iErrorCode, sResponse.c_str());
   ...
   */
   ...
   SMTPConnection::ParseAddressWithExtensions_(String mailFrom, String &address, String &parameters)
   {
      ...
      int firstQuotePosition = mailFrom.Find(PLATFORM_STRING("\""));
      if (firstQuotePosition >= 0)
      {
         int lastQuotePosition = mailFrom.ReverseFind(PLATFORM_STRING("\""));

         if (firstQuotePosition == lastQuotePosition)
            return false;

         parameterStartPosition = mailFrom.Find(PLATFORM_STRING(" "), lastQuotePosition);
      }
      else
      {
         parameterStartPosition = mailFrom.Find(PLATFORM_STRING(" "));
      }
      ...
      parameters = parameterStartPosition > 0 ? mailFrom.Mid(parameterStartPosition) : PLATFORM_STRING("");

      if (address.StartsWith(PLATFORM_STRING("<")))
      {
         if (!address.EndsWith(PLATFORM_STRING(">")))
            return false;
