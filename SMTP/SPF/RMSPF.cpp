...
initspf(int multithreaded)
{
   ...
   if (multithreaded > 0) // not called by SPFInit()
   {
      ...
      hMutex = CreateMutex(NULL, false, PLATFORM_STRING("RMSPFMutex"));
      if (hMutex == NULL)
         return false;
   ...
   hmodDnsApi = LoadLibrary(PLATFORM_STRING("DNSAPI.DLL"));
   if (hmodDnsApi == NULL)
   {
      initialized = -1;
      if (multithreaded > 0)
      ...
      for (dnsp = dnsresp; dnsp != NULL; dnsp = dnsp->pNext)
      {
         ...
         if (domain != NULL) // additional section
         {
            if (dnsp->Flags.S.Section != DNSREC_ADDITIONAL)
               continue;
            if (_stricmp((char*)dnsp->pName, domain) != 0)
               continue;
         }
         else // answer section
         {
         ...
         for (;;)
         {
            ...
            for (dnsp = dnsresp; dnsp != NULL; dnsp = dnsp->pNext)
            {
               ...
               if (datalen==SPFEXPSIZE || datap->dns_expire>dnsp->dwTtl)
                  datap->dns_expire = dnsp->dwTtl;
#endif //SPFCACHE
               cp = (char*) dnsp->Data.PTR.pNameHost;
               if (*cp != 0)
               {
                  len = (int)strlen(cp) + 1;
...
#else //SPFCACHE
                  cp = (char*) dnslistp[i]->Data.MX.pNameExchange;
#endif //SPFCACHE
                  if (*cp != 0)
                  {
                     len = (int)strlen(cp) + 1;
...
getspf(const char* domain, const dnsrec** datapp, time_t currtime)
{
   ...
   for (dnsp = dnsresp; dnsp != NULL; dnsp = dnsp->pNext)
   {
      ...
      cp = (char*) dnsp->Data.TXT.pStringArray[0];
      if (!checkversion(cp))
         continue;
      ...
      else
      {
         ...
         for (i = 0; i < dnsp->Data.TXT.dwStringCount; i++)
            datalen += (int)strlen((char*)dnsp->Data.TXT.pStringArray[i]);
         datap = (dnsrec*)spfmalloc(datalen);
         if (datap == NULL)
         {
            result = SPF_NoMemory;
         ...
         for (i = 0; i < dnsp->Data.TXT.dwStringCount; i++)
         {
            len = (int)strlen((char*)dnsp->Data.TXT.pStringArray[i]);
