...
namespace HM
{
   ...
   SMTPMessageHeaderCreator::Create()
   {
      ...
      {
         String sTemp;
         sTemp.Format(PLATFORM_STRING("Message-ID: %s\r\n"), Utilities::GenerateMessageID().c_str());
   ...
   SMTPMessageHeaderCreator::GenerateReceivedHeader_(const String &overriden_received_ip)
   {
      ...
      if (is_tls_)
         cipher_line.Format(PLATFORM_STRING("\t(version=%s cipher=%s bits=%d)\r\n"), String(cipher_info_.GetVersion()).c_str(), String(cipher_info_.GetName()).c_str(), cipher_info_.GetBits());

      String sResult;
      sResult.Format(PLATFORM_STRING("Received: from %s (%s [%s])\r\n")
         PLATFORM_STRING("\tby %s with ESMTP%s\r\n")
         PLATFORM_STRING("%s")
         PLATFORM_STRING("\t; %s\r\n"),
